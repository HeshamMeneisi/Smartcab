...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            self.t += 1
            self.epsilon = 0.994 ** self.t  # 1 / (1+math.log(self.t**3))
    ...
    def get_maxQ(self, state):
        """ The get_argmax_Q function is called when the agent is asked to find the
    ...
    def createQ(self, state):
        ...
        if self.learning:
            if not state in self.Q.keys():
                row = dict(zip(self.valid_actions, [0, 0, 0, 0]))
                self.Q[state] = row
...
def run():
    ...
    sim = Simulator(env, update_delay=update_delay, log_metrics=log_metrics,
                    display=display, optimized=True)
    ...
    pickle.dump(env.primary_agent.Q, open('./q.dat'))
